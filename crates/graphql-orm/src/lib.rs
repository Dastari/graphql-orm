    ...
    pub mod orm {
        ...
        impl SchemaModel {
            pub fn from_entities(entities: &[&EntityMetadata]) -> Self {
                Self {
                    tables: entities.iter().map(|entity| TableModel::from(*entity)).collect(),
                }
            }
        }
        ...
        pub enum MigrationStep {
            ...
            AddColumn { table_name: String, column: ColumnModel },
            ...
            CreateIndex { table_name: String, index: IndexMetadata },
        ...
        fn render_sqlite_table_rebuild_statements(
            ...
            let target_table_sql =
                render_create_table_statement_for_name(target_table, &temp_table_name);
                            ...
                            if rebuild_tables.contains(table_name) {
                                if rebuilt_tables.insert(table_name.to_string()) {
                                    if let (Some(current_table), Some(target_table)) =
                                        (current_tables.get(table_name), target_tables.get(table_name))
                                    {
        ...
        pub fn write_migration_file(
            ...
            let path = directory.as_ref().join(migration_filename(version, description));
        ...
        fn filter_expression_from_raw_parts(
            ...
            let filters = conditions
                ...
                .map(|clause| {
                    ...
                    let clause_values =
                        value_iter.by_ref().take(placeholder_count).collect::<Vec<_>>();
        ...
        pub fn render_select_query(dialect: DatabaseBackend, query: &SelectQuery) -> RenderedQuery {
            ...
            if let Some(filter) = &query.filter {
                let where_sql =
                    render_filter_expression(dialect, filter, &mut next_index, &mut values);
