...
| `provider-codex-app-server` | no | Strict run-scoped Codex app-server lifecycle, protected retained threads, explicit never-approval/read-only thread policy, disabled-only remote-control status admission, and default-off coordinator-owned experimental dynamic tools; no remote control, shell, filesystem, web, or generic JSON-RPC |
...
```toml
[dependencies]
graphql-orm-ai = { git = "https://github.com/Dastari/graphql-orm.git", rev = "<reviewed-full-40-character-commit-sha>", version = "0.72.0", features = ["sqlite"] }
```
