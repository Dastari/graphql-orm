# Documentation

This directory contains the project documentation that is too detailed for the repository front page.

- [Getting started](getting-started.md)
- [Backend features and multi-backend workspaces](backends.md)
- [Entities, keys, columns, naming, and relations](entities-and-relations.md)
- [Auth integration](auth.md)
- [Strict authorization](strict-authorization.md)
- [agql-auth bridge](agql-auth-bridge.md)
- [Cross-backend tenant isolation](cross-backend-tenant.md)
- [Public error codes](error-codes.md)
- [Pagination migration](pagination-migration.md)
- [PostgreSQL RLS and auth-aware execution](postgres.md)
- [Microsoft SQL Server read-only backend](mssql.md)
- [Federation operation roots](federation.md)
- [Schema ownership, validation, planning, and ABI migrations](schema-management.md)
- [Owned runtime schema IR](runtime-schema-ir.md)
- [Runtime values, records, handles, and row decoding](runtime-records.md)
- [Runtime select, filter, order, and keyset reads](runtime-queries.md)
- [Validated runtime relation batching](runtime-relations.md)
- [Runtime writes, hooks, subscriptions, and policies](runtime-and-writes.md)
- [Repository-only persisted entities](repository-only-entities.md)
- [Bounded append-only retention maintenance](retention-maintenance.md)
- [Backup runtime API](backup.md)
- [Schema modules and fenced leases](schema-modules-and-leases.md)
- [Release notes](release-notes.md)
- [Development and tests](development.md)

The root [README](../README.md) is the best starting point if you are new to the project.
