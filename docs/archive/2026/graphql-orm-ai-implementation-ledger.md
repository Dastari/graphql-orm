...
> **Archived implementation ledger (2026).** This long-form record preserves
> the implementation evidence and incomplete-state claims made at the time.
> It is not the current status page; use the concise replacement and the
> [AI production-readiness plan](../../plans/active/ai-production-readiness/README.md)
> for current guidance. Historical versions and checkpoint details below are
> intentionally unchanged.

This file is intentionally explicit about what is a compiled contract versus
production-ready behavior.

The [checkpoint-based completion plan](graphql-orm-ai-completion-ledger.md) defines the active
...
- Provider-persistent file upload/search, plus richer provider file-type
  ...
  contract is complete in [provider-persistent files](../../../crates/graphql-orm-ai/docs/provider-files.md).
...
[completion plan](graphql-orm-ai-completion-ledger.md). The leading priorities are:
...
4. Keep provider-persistent upload/search closed until provider create
   ...
   [provider-persistent files](../../../crates/graphql-orm-ai/docs/provider-files.md).
...
- `git diff --check` and the one-source dependency audit pass. The Slice 2-7
  ...
  [backend capability matrix](../../../crates/graphql-orm-ai/docs/backend-capability-matrix.md) remains the
