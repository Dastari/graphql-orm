...
Enable `auth-agql` for optional converters. See
[the agql-auth bridge](../reference/graphql-orm/agql-auth-bridge.md).
...
See [strict authorization](../reference/graphql-orm/strict-authorization.md).
...
[error codes](../reference/graphql-orm/error-codes.md).

## Assurance and transport boundaries

Authentication transport remains host-owned: the ORM does not implement login,
JWT validation, HTTP cookie policy, WebSocket credential refresh, identity
provider callbacks, or MFA ceremonies. It consumes only the request principal
and evidence supplied through reviewed integration contracts.

Operation assurance is an additional server-side requirement for an already
authenticated and authorized operation. It does not grant authorization, and
its directive/client metadata is advisory. See
[operation assurance](operation-assurance.md) and
[ADR-0004](../decisions/ADR-0004-authentication-authorization-and-assurance-boundaries.md).
